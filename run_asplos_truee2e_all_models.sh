...
BENCHMARK_PY="${SCRIPT_DIR}/benchmark_scaling_true_e2e.py"
...
LOG_ROOT="${SCRIPT_DIR}/results/asplos_truee2e_logs/${RUN_TAG}"
...
preflight() {
  ...
  if [[ "${CACHE_CONDITION}" == "cold" ]]; then
    if ! drop_os_page_cache "preflight"; then
      die "Cold-cache execution requires non-interactive drop_caches permission (root or sudo -n)."
    fi
  fi
}
...
run_benchmark_job() {
  ...
  local stage="$2"
  local index="$3"
  local output_json="${SCRIPT_DIR}/results_scaling_truee2e_${model}_${stage}_${RUN_TAG}.json"
...
run_artifact_builds() {
  ...
  for stage in "${artifact_stages[@]}"; do
    local out_dir="${SCRIPT_DIR}/figures/asplos_truee2e_${stage}_${RUN_TAG}"
...
main() {
  ...
  log "Overnight ASPLOS true-E2E run started"
  log "RUN_TAG=${RUN_TAG}"
