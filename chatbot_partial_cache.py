...
class ProgressiveChatbotPartial:
    ...
    def _trigger_partial_recompute(self):
        ...
        # max_tokens=1: 실제 응답 필요 없고, forward pass만 필요
        minimal_params = SamplingParams(
            temperature=0.0,
            max_tokens=1,
        )

        print(f"  Starting partial recomputation...")
        t0 = time.time()

        # 이 generate() 호출 시 partial recompute가 실행됨
        self.llm.generate([prompt], minimal_params)

        elapsed = time.time() - t0
        print(f"  ✅ Partial recomputation complete ({elapsed:.2f}s)")
        print(f"  📌 KV cache updated for current conversation state")
        print(f"  📌 Boundary cleared (next turn will be normal forward)\n")
